...
def main(cfg: DictConfig):
    ...
    print("Old model parameters:", sum(p.numel() for p in model_old.parameters() if p.requires_grad))

    # Print embedding, head, and transformer parameters
    embedding_params = sum(p.numel() for n, p in model_old.named_parameters() if 'emb' in n and p.requires_grad)
    head_params = sum(p.numel() for n, p in model_old.named_parameters() if 'head' in n and p.requires_grad)
    transformer_params = sum(p.numel() for n, p in model_old.named_parameters() if ('transformer' in n or 'blocks' in n) and p.requires_grad)
