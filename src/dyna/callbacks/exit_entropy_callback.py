...
class ExitEntropyCallback(Callback):
    ...
    def _compute_binary_entropy(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        entropy_per_position = -probs * torch.log(probs) - (1 - probs) * torch.log(
            1 - probs
        )
    ...
    def batch_end(self, state: State, logger: Logger) -> None:
        ...
        if batch_entropy:
            ...
            entropy_tensor = (
                torch.stack(batch_entropy)
                if len(batch_entropy) > 1
                else batch_entropy[0]
            )
            last_token_tensor = (
                torch.stack(batch_last_token_entrp)
                if len(batch_last_token_entrp) > 1
                else batch_last_token_entrp[0]
            )

            metrics_dict["metrics/exit_entropy"] = entropy_tensor.mean().item()
            metrics_dict["metrics/last_token_exit_entropy"] = (
                last_token_tensor.mean().item()
            )

            metrics_dict["entropy/exit_entropy"] = self._fig_to_wandb_image(
                self._create_entropy_plot(batch_entropy)
            )
            metrics_dict["entropy/last_token_exit_entropy"] = self._fig_to_wandb_image(
                self._create_entropy_plot(batch_last_token_entrp)
            )
