    ...
    def _collect_expert_selections(self, model) -> dict[str, torch.Tensor]:
        ...
        for _, batch in enumerate(data):
            for idx, elem in enumerate(batch):
                ...
                attn_o_count = torch.bincount(
                    attn_o_int, minlength=self.attn_experts
                ).to(torch.float32)
                attn_v_count = torch.bincount(
                    attn_v_int, minlength=self.attn_experts
                ).to(torch.float32)
                ffn_count = torch.bincount(ffn_int, minlength=self.ffn_experts).to(
                    torch.float32
                )
    ...
    def batch_end(self, state: State, logger: Logger) -> None:
        ...
        if self.run_data is None:
            ...
            for key in self.run_data:
                if key in selections:
                    self.run_data[key] = self.run_data[key] + torch.sum(
                        selections[key], dim=0
                    )

        # Create visualizations
        heat_map_attn_o, load_balance_attn_o = self._create_expert_heatmap(
            selections["attn_o"]
        )
        heat_map_attn_v, load_balance_attn_v = self._create_expert_heatmap(
            selections["attn_v"]
        )
        ...
        load_balance_attn_o_tot = torch.nn.functional.kl_div(
            torch.nn.functional.log_softmax(
                self.run_data["attn_o"] / sum_attn_o, dim=0
            ),
        ...
        load_balance_attn_v_tot = torch.nn.functional.kl_div(
            torch.nn.functional.log_softmax(
                self.run_data["attn_v"] / sum_attn_v, dim=0
            ),
        ...
        metrics_dict = {}
        metrics_dict[f"{self.log_key_prefix}/ffn_layer"] = self._fig_to_wandb_image(
            heat_map_ffn
        )
        metrics_dict[f"{self.log_key_prefix}/attn_o_layer"] = self._fig_to_wandb_image(
            heat_map_attn_o
        )
        metrics_dict[f"{self.log_key_prefix}/attn_v_layer"] = self._fig_to_wandb_image(
            heat_map_attn_v
        )

        metrics_dict["metrics/load_balance_attn_o"] = load_balance_attn_o.item()
        metrics_dict["metrics/load_balance_attn_v"] = load_balance_attn_v.item()
        metrics_dict["metrics/load_balance_ffn"] = load_balance_ffn.item()

        metrics_dict["metrics/load_balance_attn_total_o"] = (
            load_balance_attn_o_tot.item()
        )
        metrics_dict["metrics/load_balance_attn_total_v"] = (
            load_balance_attn_v_tot.item()
        )
        metrics_dict["metrics/load_balance_total_ffn"] = load_balance_ffn_tot.item()
