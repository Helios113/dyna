...
from dyna.cvmm.cvmm_sel import CVMMSel, cvmm_prepare_sel
...
def cvmm(
    x: torch.Tensor, sel: torch.Tensor | CVMMSel, keys: torch.Tensor
) -> torch.Tensor:
    ...
    if (
        ...
        print("---------------------------------------------------------------------")
        raise RuntimeError(
            "PyTorch >= 2.2 Triton with AMP is to be broken on Volta GPUs."
        )
    ...
    return CVMM.apply(
        x, sel.sel_index, sel.sel, keys, sel.out_index, sel.reduction_weight
    )
