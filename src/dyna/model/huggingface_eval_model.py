...
__all__ = ['HuggingFaceEvalModel', 'peft_installed']



class HuggingFaceEvalModel(ComposerModel, LightevalModel):
    """
    A wrapper class that converts 🤗 Transformers models to composer models and light eval.
    ...
    Args:
        ...
        tokenizer (transformers.PreTrainedTokenizer, optional): The tokenizer used to prepare the dataset. Default ``None``.
            ...
            .. note:: If the tokenizer is provided, its config will be saved in the composer checkpoint, and it can be reloaded
                using :meth:`HuggingFaceEvalModel.hf_from_composer_checkpoint`. If the tokenizer is not provided here, it will not be saved in the composer checkpoint.
        use_logits (bool, optional): If True, the model's output logits will be used to calculate validation metrics. Else, metrics will be inferred from the HuggingFaceEvalModel directly. Default: ``False``
    ...
    .. testcode::
       :skipif: True
        ...
        import transformers
        from composer.models import HuggingFaceEvalModel
        ...
        model = HuggingFaceEvalModel(hf_model, hf_tokenizer)
    """

    def __init__(
        self,
        model: transformers.PreTrainedModel | 'PeftModel',
        tokenizer: transformers.PreTrainedTokenizer | transformers.PreTrainedTokenizerFast | None = None,
        use_logits: bool | None = False,
        metrics: Sequence[Metric] | None = None,
        eval_metrics: Sequence[Metric] | None = None,
        ...
        if peft_config is not None:
            ...
            if peft_config.peft_type != 'LORA':
                raise ValueError(
                    f'PEFT type {peft_config.peft_type} is not supported by HuggingFaceEvalModel. Only LORA is supported.',
                )
        ...
        self.labels: torch.Tensor | None = None  # set in eval_forward() if exists
        self.dummy_forward_called = False  # Used to make FSDP generate work, see generate function for more details
        self.train_metrics: dict | None = self._get_metric_dict(metrics) if metrics is not None else None
        self.val_metrics: dict | None = self._get_metric_dict(
            eval_metrics,
        ...
        self._eval_add_special_tokens: bool = True
        ...
        if self.tokenizer is not None and self.config.vocab_size < len(self.tokenizer):
            if allow_embedding_resizing:
                ...
                raise ValueError(
                    ...
                    f' by calling `model.resize_token_embeddings(len(tokenizer))` before calling the `HuggingFaceEvalModel`'
    ...
    def load_huggingface_tokenizer_from_saved_state(
        ...
        tokenizer_save_dir: str | None = None,
    ...
    def hf_from_composer_checkpoint(
        checkpoint_path: str,
        model_instantiation_class: type[transformers.PreTrainedModel] | type['_BaseAutoModelClass'] | str | None = None,
        model_config_kwargs: dict | None = None,
        local_checkpoint_save_location: Path | str | None = None,
        trust_remote_code: bool = False,
    ) -> tuple[
        transformers.PreTrainedModel,
        transformers.PreTrainedTokenizer | transformers.PreTrainedTokenizerFast | None,
        ...
        .. testsetup::
            ...
            composer_model = HuggingFaceEvalModel(hf_model, tokenizer=hf_tokenizer, metrics=[], use_logits=True)
        ...
        .. testcode::
            ...
            hf_model, hf_tokenizer = HuggingFaceEvalModel.hf_from_composer_checkpoint('composer-hf-checkpoint.pt')
            # At this point, hf_model is randomly initialized
            composer_model = HuggingFaceEvalModel(hf_model, hf_tokenizer)
            trainer = Trainer(model=composer_model,
                              train_dataloader=train_dataloader,
        ...
        hf_tokenizer = HuggingFaceEvalModel.load_huggingface_tokenizer_from_saved_state(hf_state, trust_remote_code)
        hf_model = HuggingFaceEvalModel.load_huggingface_model_from_saved_state(
            hf_state,
            loaded_state_dict,
    ...
    # --- LightevalModel interface implementation ---
    ...
    def add_special_tokens(self) -> bool:
        ...
        return self._eval_add_special_tokens

    @property
    def max_length(self) -> int:
        """Return the maximum sequence length."""
        if self._eval_max_length is None:
            raise RuntimeError("max_length not configured. Call configure_eval_helpers first.")
        return self._eval_max_length
    ...
    def configure_eval_helpers(
        self,
        *,
        max_length: int,
        add_special_tokens: bool = True,
    ) -> None:
        """Configure evaluation helper parameters.
        
        Args:
            max_length: Maximum sequence length for evaluation
            add_special_tokens: Whether to add special tokens during tokenization
        """
        self._eval_max_length = max_length
        self._eval_add_special_tokens = add_special_tokens

    def _run_greedy_generation(
        self,
        input_ids: torch.Tensor,
        max_new_tokens: int,
    ) -> torch.Tensor:
        """Run greedy generation from input_ids."""
        ...
        for _ in range(max_new_tokens):
            ...
            with torch.no_grad():
                ...
                next_token_logits = outputs["logits"][:, -1, :] if self.config.use_return_dict else outputs[0][:, -1, :]
                next_tokens = next_token_logits.argmax(dim=-1, keepdim=True)

            generated = torch.cat([generated, next_tokens], dim=-1)

            # Check if all sequences have generated EOS token
            if self.tokenizer.eos_token_id is not None and (next_tokens == self.tokenizer.eos_token_id).all():
                break
    ...
    def _truncate_context(
        ...
        """Truncate context if total length exceeds max_length."""
        total_len = len(context) + len(continuation)
        if total_len <= self.max_length:
            return context, continuation

        overflow = total_len - self.max_length
        if overflow >= len(context):
            overflow = len(context) - 1
        trimmed_context = context[overflow:]
        return trimmed_context, continuation
    ...
    def _score_continuation(
        ...
        context_tokens, continuation_tokens = self._truncate_context(context_tokens, continuation_tokens)
        ...
        with torch.no_grad():
            ...
            logits = outputs["logits"] if self.config.use_return_dict else outputs[0]
        ...
        for offset, token_id in enumerate(continuation_tokens):
            ...
            if position < 0 or position >= log_probs.shape[1]:
                continue
            total_logprob += log_probs[0, position, token_id].item()
            predicted = logits[0, position].argmax().item()
            if predicted != token_id:
                is_greedy = False

        return float(total_logprob), is_greedy

    def _compute_rolling_logprobs(self, tokens: list[int]) -> tuple[list[float], list[bool]]:
        """Compute rolling log probabilities for a sequence."""
        if len(tokens) < 2:
            return [], []
        ...
        while start < len(tokens) - 1:
            ...
            window = torch.tensor(tokens[start:end], device=device).unsqueeze(0)
            ...
            for idx in range(window_len - 1):
                token_id = window[0, idx + 1].item()
                logprob_values.append(log_probs[0, idx, token_id].item())
                greedy_matches.append(logits[0, idx].argmax().item() == token_id)

            if end == len(tokens):
                break
            start = end - 1

        return logprob_values, greedy_matches

    def greedy_until(
        self,
        docs: Sequence["Doc"],
    ) -> list["ModelResponse"]:
        """Generate text using greedy decoding until stop sequences are encountered."""
        from lighteval.models.model_output import ModelResponse
        ...
        for doc in docs:
            ...
            max_new_tokens = doc.generation_size or 64
            stop_sequences = doc.stop_sequences or []
            num_samples = max(1, doc.num_samples)
            
            decoded_generations: list[str] = []
            generated_token_lists: list[list[int]] = []

            if num_samples > 1:
                log.warning("num_samples>1 not fully supported for greedy decoding; returning identical samples")

            for _ in range(num_samples):
                generated = self._run_greedy_generation(encoded["input_ids"], max_new_tokens)
                
                input_length = encoded["input_ids"].shape[1]
                continuation_tokens = generated[0, input_length:].tolist()
                decoded_text = self.tokenizer.decode(continuation_tokens, skip_special_tokens=True)
                
                # Apply stop sequences
                for stop_seq in stop_sequences:
                    if stop_seq and stop_seq in decoded_text:
                        decoded_text = decoded_text.split(stop_seq)[0]
                
                decoded_generations.append(decoded_text)
                generated_token_lists.append(continuation_tokens)

            results.append(
                ModelResponse(
                    text=decoded_generations,
                    output_tokens=generated_token_lists,
    ...
    def loglikelihood(
        ...
        for doc in docs:
            ...
            # Encode context and continuations
            context_encodings, continuation_encodings = self.tok_encode_pair(
                ctx,
                choices,
                pairwise=True,
            )

            doc_logprobs: list[float] = []
            doc_greedy: list[bool] = []
            doc_generations: list[list[int]] = []

            for context_tokens, continuation_tokens in zip(context_encodings, continuation_encodings):
                logprob, is_greedy = self._score_continuation(context_tokens, continuation_tokens)
                doc_logprobs.append(logprob)
                doc_greedy.append(is_greedy)
                doc_generations.append(continuation_tokens)

            responses.append(
                ModelResponse(
                    logprobs=doc_logprobs,
                    argmax_logits_eq_gold=doc_greedy,
                    input_tokens=context_encodings[0] if context_encodings else [],
                    output_tokens=doc_generations,
                )
            )
    ...
    def loglikelihood_rolling(
        ...
        for doc in docs:
            ctx = doc.ctx
            tokens = self.tok_encode(ctx, add_special_tokens=self.add_special_tokens)
            
            logprob_values, greedy_flags = self._compute_rolling_logprobs(tokens)
            
            responses.append(
                ModelResponse(
                    logprobs=logprob_values,
                    argmax_logits_eq_gold=greedy_flags,
                    input_tokens=tokens,
    ...
    def tok_encode(
        ...
        Args:
            text: Text to encode
            add_special_tokens: Whether to add special tokens. Uses instance setting if None.
            
        Returns:
            List of token IDs
        """
        if self.tokenizer is None:
            raise ValueError("Tokenizer is not configured")
    ...
    def tok_encode_pair(
        ...
        """Encode context and continuation pairs.
        
        Args:
            context: The context string
            continuations: List of continuation strings
            pairwise: If True, tokenize context+continuation together then split
            
        Returns:
            Tuple of (context_encodings, continuation_encodings)
        """
        if self.tokenizer is None:
            raise ValueError("Tokenizer is not configured")
        
        context_encodings: list[list[int]] = []
        continuation_encodings: list[list[int]] = []
        
        for continuation in continuations:
            if pairwise:
                # Tokenize the full context + continuation together
                full_tokens = self.tok_encode(
                    context + continuation,
                    add_special_tokens=self.add_special_tokens,
                )
                # Tokenize just the context to find the split point
                context_tokens = self.tok_encode(
                    context,
                    add_special_tokens=self.add_special_tokens,
                )
                # Split at the context length
                context_len = len(context_tokens)
                context_encodings.append(full_tokens[:context_len])
                continuation_encodings.append(full_tokens[context_len:])
            else:
                # Tokenize separately
                context_tokens = self.tok_encode(
                    context,
                ...
                continuation_tokens = self.tok_encode(
                    continuation,
                    add_special_tokens=False,  # Don't add special tokens to continuation
                )
                context_encodings.append(context_tokens)
                continuation_encodings.append(continuation_tokens)
...
def maybe_get_underlying_model(
    model: transformers.PreTrainedModel | 'PeftModel',
) -> transformers.PreTrainedModel | 'PeftModel':
...
def _is_registered_causal_lm(model: transformers.PreTrainedModel | 'PeftModel') -> bool:
...
def get_peft_config_from_composer_state_dict(state_dict: dict[str, Any]) -> 'PeftConfig' | None:
...
def write_huggingface_pretrained_from_composer_checkpoint(
    ...
    hf_tokenizer = HuggingFaceEvalModel.load_huggingface_tokenizer_from_saved_state(hf_state)
