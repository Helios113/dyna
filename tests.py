...
def moe_matmul_kernel(
    ...
    a_ptr, b_ptr, c_ptr,
    expert_selection_ptr, coeff_ptr,
    # Matrix dimensions
    bs, seq_len, n_heads, d_model, d_head, n_experts, k_experts,
    # Strides
    stride_a_m, stride_a_k, stride_a_unused,
    stride_b_expert, stride_b_d_model, stride_b_d_head,
    stride_c_bs, stride_c_seq, stride_c_n_heads, stride_c_d_head,
    stride_expert_bs, stride_expert_seq, stride_expert_n_heads, stride_expert_k,
    stride_coeff_bs, stride_coeff_seq, stride_coeff_n_heads, stride_coeff_k,
    # Meta-parameters
    ...
    num_pid_m = tl.cdiv(seq_len * bs if MODE == 0 else seq_len * bs * n_heads, BLOCK_SIZE_M)
    ...
    for k_idx in range(k_experts):
        ...
        for k in range(0, tl.cdiv(d_model if MODE == 0 else d_head, BLOCK_SIZE_K)):
            ...
            for m_offset in range(BLOCK_SIZE_M):
                ...
                expert_ptr = expert_selection_ptr + b_idx * stride_expert_bs + s_idx * stride_expert_seq + h_idx * stride_expert_n_heads + k_idx * stride_expert_k
                expert_idx = tl.load(expert_ptr)

                if MODE == 0:
                    actual_expert_idx = h_idx * n_experts + expert_idx
                    b_offset = actual_expert_idx * stride_b_expert + (k * BLOCK_SIZE_K + offs_k[:,None]) * stride_b_d_model + offs_n[None,:] * stride_b_d_head
                else: # MODE == 1
                    actual_expert_idx = h_idx * n_experts + expert_idx
                    b_offset = actual_expert_idx * stride_b_expert + (k * BLOCK_SIZE_K + offs_k[:,None]) * stride_b_d_model + offs_n[None,:] * stride_b_d_head
                ...
                expert_accumulator[m_offset, :] += tl.dot(a[m_offset, :], b, out_dtype=tl.float32)
        ...
        if HAS_COEFF:
             for m_offset in range(BLOCK_SIZE_M):
                ...
                coeff_ptr_row = coeff_ptr + b_idx * stride_coeff_bs + s_idx * stride_coeff_seq + h_idx * stride_coeff_n_heads + k_idx * stride_coeff_k
    ...
    if MODE == 0:
        ...
        for h in range(n_heads):
             c_ptrs = c_ptr + stride_c_bs * (offs_cm[:, None] // seq_len) + \
        ...
        c_ptrs = c_ptr + stride_c_bs * (offs_cm[:, None] // (seq_len * n_heads)) + \
            stride_c_seq * ((offs_cm[:, None] % (seq_len * n_heads)) // n_heads) + \
            offs_cn[None, :] * 1 # This assumes C is contiguous in d_model
        c_mask = (offs_cm[:, None] < bs * seq_len * n_heads) & (offs_cn[None, :] < d_model)
        tl.store(c_ptrs, c, mask=c_mask)
...
def moe_matmul(a, b, expert_selection, coeff=None):
    ...
    grid = lambda META: (
        triton.cdiv(a.shape[0], META['BLOCK_SIZE_M']) * triton.cdiv(c.shape[-1], META['BLOCK_SIZE_N']),
    )

    # Kernel call
    moe_matmul_kernel[grid](
        a, b, c, expert_selection, coeff,
        bs, seq_len, n_heads, d_model, d_head, n_experts, k_experts,
        a.stride(0), a.stride(1), a.stride(2) if len(a.shape) > 2 else 0, # a_strides
        b.stride(0), b.stride(1), b.stride(2), # b_strides
        c.stride(0), c.stride(1), c.stride(2), c.stride(3) if len(c.shape) > 3 else 0, # c_strides
        expert_selection.stride(0), expert_selection.stride(1), expert_selection.stride(2), expert_selection.stride(3),
        coeff.stride(0), coeff.stride(1), coeff.stride(2), coeff.stride(3) if coeff is not None else 0,
...
def test_moe_matmul():
    ...
    expert_matrix0 = torch.randn(n_heads * n_experts_attn, d_model, d_head, device='cuda', dtype=torch.float16)
    expert_selection0 = torch.randint(0, n_experts_attn, (bs, seq_len, n_heads, k_experts), device='cuda').to(torch.int32)
    coefficients0 = torch.rand(bs, seq_len, n_heads, k_experts, device='cuda', dtype=torch.float16)

    # Reference calculation (PyTorch)
    output_ref0 = torch.zeros(bs, seq_len, n_heads, d_head, device='cuda', dtype=torch.float16)
    for b in range(bs):
    ...
    # Triton calculation
    output_triton0 = moe_matmul(input0, expert_matrix0, expert_selection0, coefficients0)
    ...
    print("\n--- Testing Mode 1 ---")
    input1 = torch.randn(bs, seq_len, n_heads, d_head, device='cuda', dtype=torch.float16)
    expert_matrix1 = torch.randn(n_heads * n_experts_attn, d_head, d_model, device='cuda', dtype=torch.float16)
    expert_selection1 = torch.randint(0, n_experts_attn, (bs, seq_len, n_heads, k_experts), device='cuda').to(torch.int32)
    coefficients1 = torch.rand(bs, seq_len, n_heads, k_experts, device='cuda', dtype=torch.float16)
        ...
        for s in range(seq_len):
            ...
            for h in range(n_heads):
                for k in range(k_experts):
                    ...
                    token_val += coefficients1[b, s, h, k] * (input1[b, s, h] @ expert_w)
            output_ref1[b, s] = token_val


    # Triton calculation
    output_triton1 = moe_matmul(input1, expert_matrix1, expert_selection1, coefficients1)
...
def test_moe_matmul_torch():
    ...
    input0 = torch.randn(bs, seq_len, d_model, device='cuda', dtype=torch.float16)
    expert_matrix = torch.randn(n_heads * n_experts_attn, d_model, d_head, device='cuda', dtype=torch.float16)
    expert_selection = torch.randint(0, n_experts_attn, (bs, seq_len, n_heads, k_experts), device='cuda').to(torch.int32)
    
    coefficients = torch.rand(bs, seq_len, n_heads, k_experts,1,1, device='cuda', dtype=torch.float16)
    
    # Reference calculation (PyTorch)
    output_ref = torch.zeros(bs, seq_len, n_heads, d_head, device='cuda', dtype=torch.float16)
    for b in range(bs):
        for s in range(seq_len):
            for h in range(n_heads):
                token_val = torch.zeros(d_head, device='cuda', dtype=torch.float16)
                for k in range(k_experts):
                    ...
                    token_val += coefficients[b, s, h, k].squeeze() * (input0[b, s] @ expert_w)
                output_ref[b, s, h] = token_val
    print("show torch memory usage before")
    print(torch.cuda.memory_allocated() / 1024**3)
    input0 = torch.einsum("bsnkdh,bsd->bsnh", expert_matrix[expert_selection]*coefficients, input0).contiguous()
    print("show torch memory usage after")
    print(torch.cuda.memory_allocated() / 1024**3)
    print(input0.shape) # Should be (b-bs, s-seq_len, n-n_heads, k-k_experts, d-d_model, h-d_head)
    # 1073741824
    print(input0)
    print(output_ref)
